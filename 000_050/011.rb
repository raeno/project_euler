...
class Matrix
	...
	def to_n_size_matrices(n)
		...
		submatrices = []
		row_index = n
		while row_index <= row_size do 
			col_index = n
			while col_index <= column_size do
...
class TestProblem_11 < Test::Unit::TestCase
	def test_greatest_product_of_four_adj_numbers_in_the_20x20_matrix
		...
		greatest_product = matrix.to_n_size_matrices(4).map { |matrix| matrix.max_product}.max
		assert_equal(70600674, greatest_product)
	end
end
